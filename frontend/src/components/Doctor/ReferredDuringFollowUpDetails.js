...
const ReferredDuringFollowUpDetails = () => {
  const location = useLocation();
  const [loading, setLoading] = useState(false);
  const { data } = location.state;
  console.log("referred during patient data", data);
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case "Update Prescriptions":
        if (loading) return <LoadingComponent />;
        return <ViewPrescription prescriptionDTO={data.prescriptionDTO} patient_id={ data.patient_id} type={"update"}/>;
      default:
        return null;
    }
  };
    ...
    <div className="font-[sans-serif]">
      ...
      <div className="mt-15">
        ...
        {isSidebarOpen && (
          <aside
            id="default-sidebar"
            className="fixed top-0 left-0 z-40 w-[340px] h-screen transition-transform -translate-x-full sm:translate-x-0"
            aria-label="Sidebar"
          >
            <div className="h-full px-3 py-4 overflow-y-auto bg-[#f5f5f9] dark:bg-gray-800 mt-[73px]">
              <ul className="space-y-2 font-medium ">
                ...
                <li>
                  <button
                    onClick={() => setCurrentPage("Update Prescriptions")}
                    className={`flex items-center p-2 rounded-lg group no-underline ${
                      currentPage === "Update Prescriptions"
                        ? "text-white bg-[#6467c0]"
                        : "text-gray-900 "
                    }`}
                  >
...
export default ReferredDuringFollowUpDetails;
  ...
  return (
    <div className=" container mx-auto rounded-lg shadow-lg p-6 flex-1 bg-[#e0e0eb]">
      ...
      {followUpDetails.map((followup, follow_index) => (
        <div
          className="bg-[#cacbd9] rounded-lg p-4 my-3 flex flex-col items-center"
          key={follow_index}
        >
          {follow_index === 0 ? (
